...
- (void)viewWillAppear:(BOOL)animated {
    ...
    _signINController.falseSignSuccBlock = ^(){
        [weakSelf configUITodayIsSign:YES signDay:weakSelf.currentSignDay complete:nil];
        return YES;
    };
}

- (void)configUITodayIsSign:(BOOL)isTodaySign signDay:(NSInteger)signDay complete:(void(^)())complete{
    [self.activityView startAnimating];
    
    __weak typeof(self)weakSelf = self;
    __block NSInteger currentSignDay = signDay;
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        for(NSInteger i = startSign; i <= currentSignDay; i ++) {
        ...
        NSString * sign_start_Str = [NSString stringWithFormat:@"%.0f", [[nowDate daysAgo:currentSignDay] timeIntervalSince1970]];
        NSDictionary * sign_historyDic = @{@"sign_start": sign_start_Str,@"sign_duration" :@(currentSignDay), @"sign_describe" : @"再连续签到xx天即可获得50人民币"};
...
- (IBAction)goToSignInButtonClick:(id)sender {
    ...
    __weak typeof(self)weakSelf = self;
    [self configUITodayIsSign:_istodaySignSwitch.isOn signDay:self.currentSignDay complete:^{
        btn.enabled = YES;
        [weakSelf.navigationController pushViewController:_signINController animated:YES];
    }];
